...
const ALLOWED_SUBHEADINGS = new Set([
  'Added', 'Changed', 'Dependencies', 'Fixed',
  'Infrastructure', 'Removed', 'Security',
]);
...
const COMMIT_ATTEMPT_RE = /\[`([a-f0-9]+)`\]\(https?:\/\/[^)]*\/commit\/[^)]*\)/g;
...
const unreleasedCount = sections.filter(s => UNRELEASED_RE.test(s.title)).length;
if (unreleasedCount === 0) {
  error('No ## [Unreleased] section found — exactly one is required');
} else if (unreleasedCount > 1) {
  error(`Found ${unreleasedCount} ## [Unreleased] sections — exactly one is required`);
}
...
for (const section of sections) {
  ...
  if (versionsSeen.has(section.title)) {
    error(`Line ${section.lineNo}: duplicate version heading [${section.title}]`);
  }
...
for (const section of sections) {
  ...
  for (const sub of subs) {
    ...
    if (seen.has(sub.title)) {
      error(`Line ${sub.lineNo}: duplicate ### ${sub.title} in ## [${section.title}]`);
    }
    seen.add(sub.title);
...
while ((anyMatch = COMMIT_ATTEMPT_RE.exec(content)) !== null) {
...
while ((match = COMMIT_REF_RE.exec(content)) !== null) {
