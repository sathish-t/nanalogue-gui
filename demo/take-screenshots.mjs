...
// AI Chat providers to test — each gets its own parameterized test.
const AI_PROVIDERS = [
    {
        name: "openai",
...
for (const provider of AI_PROVIDERS) {
    test(`AI Chat screenshots (${provider.name})`, async () => {
        const apiKey = process.env[provider.keyEnv];
        if (!apiKey) {
            test.skip(
                true,
                `${provider.keyEnv} not set — skipping ${provider.name} AI Chat screenshots`,
            );
            return;
        }
        ...
        await page.fill("#input-endpoint", provider.endpoint);
        ...
        await page.fill("#input-model", provider.model);
        await page.waitForTimeout(5000);
        ...
        await page.screenshot({
            path: resolve(
                dir,
                `screenshot-ai-chat-configured-${provider.name}.png`,
            ),
        });
        ...
        // Grant consent for the remote endpoint on the first send.
        ...
        for (let i = 0; i < questions.length; i++) {
            ...
            await page.screenshot({
                path: resolve(
                    dir,
                    `screenshot-ai-chat-q${i + 1}-${provider.name}.png`,
                ),
            });
        }
        ...
        await page.screenshot({
            path: resolve(
                dir,
                `screenshot-ai-chat-sandbox-${provider.name}.png`,
            ),
        });
