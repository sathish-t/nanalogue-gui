...
- `plot_series(points, kind, **kwargs)` external tool in the AI Chat sandbox: renders x/y point data as a line or scatter SVG using Vega-Lite server-side; accepts `{"x": ..., "y": ...}` dicts with finite numbers, `kind` of `"line"` or `"scatter"` (default `"line"`), optional `xlabel`, `ylabel`, `title`, `xlim`, `ylim`; auto-generates output path in `ai_chat_output/` when none supplied; all path safety delegated to `write_file`; backed by a new `xy-renderer.ts` pure renderer ([`2eeeb7f`](https://github.com/sathish-t/nanalogue-gui/commit/2eeeb7facdaf063d19e9f9fde09ff681d0a5d09a))
- Showboat demos for `plot_histogram` and `plot_series` in `demo/showboat-notes/`: covers `read_info`, `bam_mods`, `window_reads`, `plot_histogram`, and `plot_series` against `demo.bam`; also adds a showboat demo for `minimap2` covering basic alignment, PAF parsing, aligning from a string, and the unmapped case ([`08366e6`](https://github.com/sathish-t/nanalogue-gui/commit/08366e63e91d93544d353a74f24357c09b656106), [`e787e86`](https://github.com/sathish-t/nanalogue-gui/commit/e787e86637b1e2cf438fee611487b63088d27bde), [`2eeeb7f`](https://github.com/sathish-t/nanalogue-gui/commit/2eeeb7facdaf063d19e9f9fde09ff681d0a5d09a))
- Extracts shared plot utilities (`autoOutputPath`, `validateLim`, `validateLabel`, `WRITE_ONLY_NOTE`) from `plot-histogram.ts` and `plot-series.ts` into a new `plot-utils.ts` module ([`5069e73`](https://github.com/sathish-t/nanalogue-gui/commit/5069e7314ec70a2889e46da6fc3ce5e4662e950c))
...
- `ls()` now always returns `list[str]`; previously returned a shape-changing dict when the entry cap was hit, causing silent breakage when iterating; now throws a `SandboxError` instead; cap raised from 500 to 10,000 entries; `maxLsEntries` wired through `SandboxOptions` ([`5a75200`](https://github.com/sathish-t/nanalogue-gui/commit/5a752003a01b7879de5eb48edb57c58eb5a2ed26))
- `bash()` and `seq_table()` execution limits now derived from `maxMemory` and `maxAllocations` sandbox config; removes the separate `maxOutputBytes` parameter from both tools; raises `FEEDBACK_OUTPUT_MAX_BYTES` from 4 KB to 10 KB ([`f0dd568`](https://github.com/sathish-t/nanalogue-gui/commit/f0dd56886277a2d175b05a8020ac492fa3597545))
- `/dump_llm_instructions` HTML transcript no longer cuts off the last assistant bubble; the exec result feedback and final user-facing answer are now mirrored into `lastSentMessages` after the terminal break ([`b853f6e`](https://github.com/sathish-t/nanalogue-gui/commit/b853f6e934388a9d33fe656d035986fa53407452), [`675495a`](https://github.com/sathish-t/nanalogue-gui/commit/675495a14058cc3b29d3dacfc57c0472e912ea4c))
- `seqTable` return type corrected in API docs from JSON string to TSV string ([`bd640cf`](https://github.com/sathish-t/nanalogue-gui/commit/bd640cfc3361c765ea75274b0470e0639318ce43))
- `MontyRuntimeError` caught in `runSandboxCode` now uses `error.display('traceback')` instead of `error.message`; the LLM now sees a full Python-style traceback (filename, line number, source preview with caret marker) rather than a bare exception message; external-function errors (`peek`, `read_info`, `bam_mods`, etc.) include the Python call site as the innermost frame; two `throw new Error()` in `bash.ts` replaced with `throw new SandboxError("OSError", ...)` so the correct Python exception type appears in the traceback rather than a generic `RuntimeError` ([`26442a8`](https://github.com/sathish-t/nanalogue-gui/commit/26442a86354f6d0c01e482705c4e30ab9f410df9))
- `SyntaxError` execution-feedback hint expanded to guide the LLM to use `print()` for direct user output and avoid misusing `continue_thinking()`; sandbox prompt gains the same instruction as a general rule; `minimap2` FASTA documentation in the prompt expanded with multi-sequence examples and `.gz` guidance ([`1f2e608`](https://github.com/sathish-t/nanalogue-gui/commit/1f2e608306a2b6833e52c0b2546dac2620bf0315))
