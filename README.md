...
- Node.js 22.16.0+ within the Node 22 release line, or Node.js 24+
...
This puts the `nanalogue-chat` and `nanalogue-sandbox-exec` commands on your PATH from
any directory. See the respective CLI sections [here](#cli-nanalogue-chat) and
[here](#cli-nanalogue-sandbox-exec) for usage.
...
endpoint.** This includes local runners like Ollama and LM
Studio, and OpenAI-compatible gateways such as OpenRouter, Mistral, Together
AI, and Fireworks. The Fetch Models button also supports native Anthropic and
Google Gemini /models endpoints. 
...
output are never sent to a language model. This makes it useful for 
reproducible analyses where LLM involvement is not needed.
...
Development environment:
  - Node.js 22.16+ within the Node 22 release line, or Node.js 24+, is required to run the app
  - Development and CI use Node 22.16 as the minimum version; Node 24 is also tested
  - The package-lock.json is generated by the pre-commit hook under the pinned Node 22 toolchain, so npm ci stays consistent across supported Node versions
...
The HTML transcripts generated by `/dump_llm_instructions` and `/dump_history`
embed the [highlight.js](https://highlightjs.org/) atom-one-dark theme CSS
inline. highlight.js is copyright © 2006 Ivan Sagalaev and contributors,
released under the [BSD 3-Clause License](https://github.com/highlightjs/highlight.js/blob/main/LICENSE).

The AI Chat `minimap2` external tool uses a WebAssembly build of
